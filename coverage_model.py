...
class Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs, coverage):
        ...
        coverage = coverage.transpose(0, 1)
        attn_energies = self.calculate_score(h, encoder_outputs, coverage)
        # a_ij = exp(e_ij) / (sigma_k(exp(e_ik)))
        return F.softmax(attn_energies, dim=1).unsqueeze(1)

    def calculate_score(self, hidden, encoder_outputs, coverage):
        # e_ij = a(s_{i-1}, h_j, c_{i-1,j}) s: hidden, h: encoder_outputs c: coverage vector
        # [B*T*2H]->[B*T*H]
        # energy = F.relu(self.attn(torch.cat([hidden, encoder_outputs], 2)))
        energy = torch.tanh(self.attn(torch.cat([hidden, encoder_outputs, coverage], 2)))
...
class Decoder(nn.Module):
    def __init__(self, embed_dim, hidden_dim, output_dim, n_layers=1, dropout=0.2):
        ...
        self.coverage_f = nn.GRU(hidden_dim+hidden_dim+hidden_dim+embed_dim, hidden_dim, n_layers)

    def forward(self, input, last_hidden, encoder_outputs, coverage):
        # Use the last output word as the current input word
        embedded = self.dropout(self.embed(input).unsqueeze(0))  # (1,B,N)
        # Calculate attention weights
        attn_weights = self.attention(last_hidden[-1], encoder_outputs, coverage)
        ...
        # C_{i,j} = f(C_{i-1,j},a_{i,j},h_{j},s_{i-1})
        new_coverage, _ = self.coverage_f(torch.cat([coverage,
                                                  attn_weights.repeat(1, coverage.size(2), 1).reshape(coverage.shape),
                                                  hidden.repeat(coverage.size(0), 1, 1), encoder_outputs], 2))

        return output, hidden, attn_weights, new_coverage
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg=None, teacher_forcing_ratio=0.5):
        ...
        # pdb.set_trace()
        coverage = Variable(torch.zeros(src.size(0), batch_size, self.hidden_dim)).to(self.device)
        ...
        for t in range(1, max_len):
            output, hidden, attn_weights, coverage = self.decoder(
                output, hidden, encoder_output, coverage)
            outputs[t] = output
